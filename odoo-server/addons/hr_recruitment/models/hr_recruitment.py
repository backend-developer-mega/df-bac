...
class RecruitmentStage(models.Model):
    ...
    email_data = fields.Selection(ROL_EMAIL, "Destinatario email", default='0')
    ...
    end_stage = fields.Boolean("Etapa final", default=False)
    start_stage = fields.Boolean("Etapa inicial", default=False)

    # @api.depends('name')
    # def _compute_start_stage(self):
    #     list_stage_career = self.env['hr.recruitment.stage'].search([('job_id', '=', self.job_id.id),('id', '!=', self.id)], order='sequence asc')
    #     _logger.info('---############## _compute_start_stage ----> %s', list_stage_career)
    #     for line in list_stage_career:
    #         _logger.info('---############## _compute_start_stage ----> %s < %s', self.sequence, line.sequence)
    #         if self.sequence < line.sequence:
    ...
    def onchange_job_id(self):
        if self._context and self._context.get('default_job_id'):
            self.job_id = self._context.get('default_job_id')
...
class Applicant(models.Model):
    ...
    name = fields.Char("Tema de grado", required=True, track_visibility='onchange')
    active = fields.Boolean("Active", default=True, help="If the active field is set to false, it will allow you to hide the case without removing it.")
    description = fields.Text("Tema de grado")
    ...
    availability = fields.Date("Availability", help="The date at which the applicant will be available to start working", track_visibility="onchange")
    ...
    student_ids = fields.Many2many('res.partner', 'students_lead_tag_rel_res', 'students_lead_id_res', 'students_tag_id_res', required=True, string="Integrantes")
    stage_check = fields.Boolean(related='stage_id.end_stage')
    teacher_director = fields.Many2one('res.partner', "Asesor", track_visibility='onchange')
    department_head = fields.Many2one('res.partner', "Jefe departamento", compute='_compute_set_value', store=True)
    ...
    def _track_subtype(self, init_values):
        ...
        elif 'department_id' in init_values and record.department_id:
            return 'hr_recruitment.mt_applicant_stage_changed'
        elif 'name' in init_values and record.stage_id.sequence > 1:
            return 'hr_recruitment.mt_applicant_stage_changed'
        elif 'teacher_director' in init_values and record.stage_id.sequence > 1:
            return 'hr_recruitment.mt_applicant_stage_changed'
    ...
    def create_employee_from_applicant(self):
        ...
        for applicant in self:
            ...
            for ids_partner in applicant.student_ids.ids: 
            ...
            for line in applicant.student_ids.ids:
                project_evaluation = self.env['project.evaluation'].create({
                    'project_task_id': project_topic.id,
                    'student_id': line
                })
                search_get_stages = self.env['project.task.type'].search([('fold', '=', False)])
                #search_get_stages = self.env['project.task.type'].search([('project_ids', '=', [7])])
                for stages in search_get_stages:
                    criterion_get = self.env['project.criterion'].search([('project_stage_id', '=', stages.id)])
                    for criterion in criterion_get:
                        project_notes = self.env['project.record.notes'].create({
                            'stage_id': stages.id,
                            'criterion_id': criterion.id,
                            'student_id': line,
                            'project_task_id': project_topic.id,
                            'evaluation_id': project_evaluation.id
                            })
                    project_resumen = self.env['project.evaluation.resumen'].create({
                        'stage_id': stages.id,
                        'student_id': line,
                        'project_task_id': project_topic.id,
                        'evaluation_id': project_evaluation.id
    ...
    @api.multi
    def create_employee_from_applicant2(self):
        employee = False
        for applicant in self:
