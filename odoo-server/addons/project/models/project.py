...
class ProjectTaskTypeCriterion(models.Model):
    ...
    project_stage_id = fields.Many2one('project.task.type', string='Etapa del projecto')

class ProjectRecordNotes(models.Model):
    _name = 'project.record.notes'
    _description = 'Registro de notas'
    _order = 'stage_id, criterion_id, student_id'

    stage_id = fields.Many2one('project.task.type', string='Evaluación')
    criterion_id = fields.Many2one('project.criterion', string='Criterio')
    student_id = fields.Many2one('res.partner', string='Egresado')
    project_task_id = fields.Many2one('project.task', string='Tema de grado')
    note = fields.Float(string='Nota', digits=(16, 2))

    evaluation_id = fields.Many2one('project.evaluation')
    percentage = fields.Float(string='% Porcentaje', digits=(16, 2), related='criterion_id.percentage')

    # @api.depends('criterion_id')
    # def _compute_get_percentage(self):
    #     self.percentage = self.criterion_id.percentage

    # @api.onchange('criterion_id')
    # def _onchange_get_percentage(self):
    #     self.percentage = self.criterion_id.percentage


class ProjectEvaluationResume(models.Model):
    _name = 'project.evaluation.resumen'
    _description = 'Resumen de los resultados de las evaluaciones'
    _order = 'stage_id, project_task_id, student_id'

    stage_id = fields.Many2one('project.task.type', string='Evaluación')
    student_id = fields.Many2one('res.partner', string='Egresado')
    project_task_id = fields.Many2one('project.task', string='Tema de grado')
    average = fields.Float(string='Promedio', digits=(16, 2))

    evaluation_id = fields.Many2one('project.evaluation')
    percentage = fields.Float(string='% Porcentaje', digits=(16, 2), related='stage_id.percentage')
    average_with_per = fields.Float(string='Global', digits=(16, 2))


class ProjectEvaluation(models.Model):
    _name = 'project.evaluation'
    _description = 'Registro de las notas de las evaluaciones'

    project_task_id = fields.Many2one('project.task', string='Tema de grado')
    student_id = fields.Many2one('res.partner', string='Egresado')
    stage_process_grade = fields.Selection([
            ('0', _('En curso')),
            ('1', _('Aprobado')),
            ('2', _('Reprobado')),
        ],
        ...
        default='0')
    average_global = fields.Float(string='Promedio global', digits=(16, 2))
    record_notes = fields.One2many('project.record.notes', 'evaluation_id', domain=lambda self: [('stage_id.id', '<=', self.project_task_id.stage_id.id)])
    resumen_average = fields.One2many('project.evaluation.resumen', 'evaluation_id')

    student_name = fields.Char("Nombre", compute="_compute_get_imagen", store=True)
    student_card = fields.Char("Carnet", compute="_compute_get_imagen", store=True)
    name = fields.Char("Nombre", compute="_compute_get_imagen", store=True)

    @api.depends('student_id')
    def _compute_get_imagen(self):
        for partner in self:
            partner.name = partner.student_id.name
            partner.student_name = partner.student_id.name
            partner.student_card = partner.student_id.student_card

    @api.onchange('record_notes')
    def _onchange_update_resumen(self):
        evaluation_resum = self.env['project.evaluation.resumen'].search([('evaluation_id.id', '=', self._origin.id)])
        evaluation_full = self.env['project.record.notes'].search([('evaluation_id.id', '=', self._origin.id)])
        self.average_global = 0.0
        for line in evaluation_resum:
            average_detail = 0.0
            for full in evaluation_full:
                if line.stage_id.id == full.stage_id.id:
                    average_detail = average_detail + (full.note * full.percentage)/100
            average_with_per = (average_detail * line.percentage) /100 
            line.write({'average': average_detail, 'average_with_per': average_with_per})
            self.average_global = self.average_global + average_with_per
...
class Task(models.Model):
    ...
    def name_get(self):
        res = []
        for record in self:
            #self._compute_set_value(record.name)
    ...
    name_value = fields.Char(string='Tema de grado', compute="_compute_set_value")
    ...
    user_id = fields.Many2one('res.users',
        string='Docente director',
        default=lambda self: self.env.uid,
        index=True, track_visibility='always')
    partner_id = fields.Many2one('res.partner',
        string='Customer',
        default=_get_default_partner)
    applicant_id = fields.Many2one('hr.applicant', string='Solicitud origen')
    ...
    def action_view_notes_server(self):
        ...
        return {
            ...
            'help': _('''<p class="oe_view_nocontent_create">
                        ...
                        Cada trabajo de grado debe tener asigandos sus integrantes.
                    </p>'''),
            'limit': 80,
