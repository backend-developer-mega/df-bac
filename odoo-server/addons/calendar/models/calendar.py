...
class Attendee(models.Model):
    ...
    def do_accept(self):
        ...
        models = 79
        template_confirm = IrModelData.xmlid_to_object('calendar.calendar_template_meeting_confirm')
        emails = ''
        email_cc = ''
        template_res_partner = self.env['res.partner']
        for ids_partner in self.event_id.partner_ids.ids:
            emails = template_res_partner.search([('id', '=',ids_partner)], limit=1).email + ',' +emails
        if emails:
            email_cc = emails[:len(emails) - 1]
        if template_confirm:
            MailTemplate = self.env['mail.template']
            body_html = MailTemplate.render_template(template_confirm.body_html, 'calendar.attendee', self.id)
            mail = self.env['mail.mail'].create({
                ...
                'partner_to': '',
                'model_id': 79,
                'subject': 'Se ha aceptado la invitación',
                'body_html': body_html,
            })
            mail.send()
        result = self.write({'state': 'accepted'})
        for attendee in self:
            attendee.event_id.message_post(body=_("%s ha aceptado la invitación.") % (attendee.common_name), subtype="calendar.subtype_invitation")
        return result
    ...
    def do_decline(self):
        ...
        IrModelData = self.env['ir.model.data']
        models = 79
        template_confirm = IrModelData.xmlid_to_object('calendar.calendar_template_meeting_refuse')
        emails = ''
        email_cc = ''
        template_res_partner = self.env['res.partner']
        for ids_partner in self.event_id.partner_ids.ids:
        ...
        if template_confirm:
            ...
            mail = self.env['mail.mail'].create({
                ...
                'partner_to': '',
                'model_id': 79,
                'subject': 'Se ha rechazado la invitación',
                'body_html': body_html,
            })
            mail.send()
        res = self.write({'state': 'declined'})
        for attendee in self:
            attendee.event_id.message_post(body=_("%s ha rechazado la invitación") % (attendee.common_name), subtype="calendar.subtype_invitation")
        return res
...
class Meeting(models.Model):
    ...
    observation = fields.Text('Observaciones', states={'done': [('readonly', True)]})
